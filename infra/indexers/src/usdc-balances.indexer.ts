...
export default function decodeUSDCBalances(
  { header, events, stateUpdate }: Block,
) {
  ...
  return Array.from(addresses).map((address) => {
    ...
    const entryLow = storageEntries.find((entry) =>
      BigInt(entry.key) === location
    );
    const entryHigh = storageEntries.find((entry) =>
      BigInt(entry.key) === location + 1n
    );
