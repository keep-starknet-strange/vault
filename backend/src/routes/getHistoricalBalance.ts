    ...
    async (request, reply) => {
      ...
      try {
        const subquery = fastify.db
          ...
          .from(usdcBalance)
          .where(
            and(eq(usdcBalance.address, address), gte(usdcBalance.blockTimestamp, sql`NOW() - INTERVAL '30 days'`)),
          )
        ...
        const historicalBalances = await fastify.db
          ...
          .innerJoin(
            subquery,
            and(eq(usdcBalance.address, subquery.address), eq(usdcBalance.blockTimestamp, sql`maxBlockTimestamp`)),
          )
